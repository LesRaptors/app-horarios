...
export default function BillingPaymentFailedEmail({ orgName, amountCop, paymentUrl }: Props) {
  return (
    <Html lang="es-CO">
      <Head />
      <Preview>No pudimos procesar el cobro de tu suscripción — Tus Horarios</Preview>
      <Body style={body}>
        <Container style={container}>
          ...
          <Section style={content}>
            <Heading style={h1}>No pudimos procesar tu pago</Heading>
            <Text style={p}>Hola, {orgName}.</Text>
            <Text style={p}>
              No pudimos procesar el cobro de tu suscripción
              {amountCop !== undefined ? ` por ${formatCop(amountCop)}` : ''}. Por favor actualiza tu método de pago para mantener el acceso a Tus Horarios.
            </Text>
            <Text style={p}>
              Si no regularizas el pago, tu cuenta quedará suspendida en los próximos días.
            </Text>
            <Button style={button} href={paymentUrl}>Actualizar tarjeta</Button>
          </Section>
...
const button = { backgroundColor: '#DC2626', color: '#FFFFFF', textDecoration: 'none', fontSize: '16px', fontWeight: 600, padding: '14px 32px', borderRadius: '8px', display: 'inline-block', marginTop: '8px' };
