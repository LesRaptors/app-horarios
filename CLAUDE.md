...
- **Horario de festivo del turno** (`shift_templates.holiday_*`): solo cambian las HORAS de un turno cuando cae en festivo (mismo turno, misma cobertura). Se define en `/shifts`. Es independiente del **perfil de festivo en Necesidades** (`staffing_requirements.is_holiday`), que cambia QUÉ se necesita (posiciones/turnos/cantidades) en festivos. La resolución vive en el helper puro `effectiveShiftHours(template, isHolidayDate)` (`equity-helpers.ts`), reusado por el motor (`pushSlot`) y la asignación manual (`shift-assign-dialog.tsx`). **Carácter nocturno efectivo:** si aplican las horas de festivo, el `is_night` efectivo se deriva de esas horas (`suggestIsNight`), no del flag almacenado; el motor usa `slot.isNight` (no `template.is_night`) en el inviolable `available_nights`, la penalización por noches, el descanso 24h post-noche y `max_consecutive_nights`. En días no festivos o turnos sin horario de festivo, `slot.isNight === template.is_night` (idéntico a hoy).
  - **Limitaciones diferidas (no arregladas):** (1) Si el horario de festivo excede el tope de horas diarias del contrato (`is_healthcare` 12h vs `maxHoursPerDay`), el turno quedará SIN CUBRIR en festivos — el motor respeta ese inviolable en ambos pases; usa un horario de festivo dentro del tope. (2) `holiday_break_minutes` (igual que `break_minutes` normal) ajusta el presupuesto interno de horas del motor pero NO se resta de las horas pagadas/equidad (nómina y rollup calculan horas brutas del rango) — mismo comportamiento que el descanso de turnos normales.
  - **Coherencia del carácter nocturno efectivo (seguimientos pendientes, no bloqueantes):** el motor usa el carácter nocturno EFECTIVO (`slot.isNight`) para scoring, rollup in-run e inviolables, pero hay capas que aún leen el flag `shift_templates.is_night`: (a) el trigger `recompute_equity_rollup` cuenta `COUNT(*) FILTER (WHERE st.is_night)` y la UI de equidad (`auto-generate-dialog`, dashboard) usan el flag — un turno de festivo con horas nocturnas pero `is_night=false` se cuenta como noche en la generación pero NO en el rollup persistido (la ventana móvil de 3 meses lo sub-cuenta → leve deriva de equidad). (b) la init del tracker desde entries existentes (`schedule-generator.ts`, `t.lastShiftWasNight = tpl?.is_night`) usa el flag, no las horas reales del entry — al regenerar, un entry de festivo-noche preexistente puede recibir 12h en vez de 24h de descanso post-noche. (c) `suggestIsNight` (solape 21:00-06:00) vs la heurística de string de `rest-rules` (`start_time>=21:00 || <06:00`) difieren para turnos vespertinos que cruzan medianoche. Cerrar (a) requiere migración (derivar noche de `start_time/end_time` en el trigger); ninguno viola ley CST.
